...
List<Rectangle> _parseString(String stringCoords) {
    ...
    for(String stringRect in stringCoords.split('|').toList()) {
      ...
      if(elements.length != 4) {
        throw ArgumentError('Must supply exactly 4 values for coordinates (x,y,width,height) ${elements.length} supplied: $stringRect');
      }
      coordinates.add(Rectangle(int.parse(elements[0]), int.parse(elements[1]), int.parse(elements[2]), int.parse(elements[3])));
    }
    return coordinates;
}

...
void main() {
  ...
  var cloudinary = Cloudinary.withStringUrl('cloudinary://a:b@test123?analytics=false');
   ...
   var cloudinaryPrivateCdn = Cloudinary.withStringUrl('cloudinary://a:b@test123?analytics=false');
   cloudinaryPrivateCdn.config.urlConfig.privateCdn = true;

  var cloudinaryPrivateCdnUseRootPath = Cloudinary.withStringUrl('cloudinary://a:b@test123?analytics=false');
  ...
  var cloudinaryPrivateCdnSignUrl = Cloudinary.withStringUrl('cloudinary://a:b@test123?analytics=false');
  cloudinaryPrivateCdnSignUrl.config.urlConfig.privateCdn = true;
  cloudinaryPrivateCdnSignUrl.config.urlConfig.signUrl = true;

  var cloudinarySignedUrl = Cloudinary.withStringUrl('cloudinary://a:b@test123?analytics=false');
  cloudinarySignedUrl.config.urlConfig.signUrl = true;

  var cloudinaryLongSignedUrl = Cloudinary.withStringUrl('cloudinary://a:b@test123?analytics=false');
  ...
  group('Test cloud name', () {
    ...
    test('Test different cloud name in options ', () {
      var cloudinaryDifferentCloud = Cloudinary.withStringUrl('cloudinary://a:b@test123?analytics=false');
  ...
  group('Test secure distribution', () {
    test('Test should use default secure distribution if secure is true', () {
      ...
      var result2 = Cloudinary.withConfiguration(newConfig).image().generate('test');
      assert('https://config.secure.distribution.com/test123/image/upload/test' == result2);
    });

    test('Test should overwrite secure distribution successfully', () {
      var cloudinarySecureDistribution = Cloudinary.withStringUrl('cloudinary://a:b@test123?analytics=false');
      cloudinarySecureDistribution.config.urlConfig.secureDistribution = 'something.else.com';
  ...
  group('Test Akamai CDN', () {
    test('Test secure true successfully produce valid URL', () {
      var cloudinarySecureWithPrivateCdn = Cloudinary.withStringUrl('cloudinary://a:b@test123?analytics=false');
    ...
    test('Test non Akamai CDN successfully produce valid URL', () {
      ...
      cloudinarySecureDistribution.config.urlConfig.secureDistribution = 'something.cloudfront.net';
      var result = cloudinarySecureDistribution.image().generate('test');
    ...
    test('Test private Cdn with http successfully produce valid URL', () {
        var cloudinaryWithPrivateCdn = Cloudinary.withStringUrl('cloudinary://a:b@test123?analytics=false');
    ...
    test('Test url config with cname produce valid URL', () {
      var cloudinarySecureFalseWithCname = Cloudinary.withStringUrl('cloudinary://a:b@test123?analytics=false');
    ...
    test('Test URL suffix with private cdn produce valid URL', () {
      ...
      // actual = cloudinaryPrivateCdn.image(options: ImageBuilder() ..urlSuffix = 'hello') ..transformation = Roate.byAngle(0) ).generate('test');
      // assert('https://test123-res.cloudinary.com/images/a_0/test/hello', actual);
    });
    ...
    group('Use root path tests', () {
      test('Test use root path with private cdn produce valid url', () {
        ...
        //TODO: Implement the following test with angle
        // actual = cloudinaryPrivateCdnUseRootPath.image(ImageBuilder() ..transformation = Roate.byAngle(0)).generate('test');
    ...
    group('Test folder', () {
      ...
      test('Test folder with exclude version produce valid url', () {
        var cloudinaryForceVersionFalse = Cloudinary.withStringUrl('cloudinary://a:b@test123?analytics=false');
    ...
    test('test shortne enabled produce valid url', () {
      var cloudinaryShortenTrue = Cloudinary.withStringUrl('cloudinary://a:b@test123?analytics=false');
    ...
    test('Test escape in public id produce valid url', () {
      var cloudinary = Cloudinary.withStringUrl('cloudinary://a:b@test123?analytics=false');
      var publicIdsToTest = {'a b': 'a%20b', 'a+b': 'a%2Bb', 'a%2Bb': 'a%2Bb', 'a-b': 'a-b', 'a??b': 'a%3F%3Fb'};
    ...
    test('Test signed url produce valid url', () {
      ...
      //TODO: Fix this test with transformation RESIZE
      // var expected = "${defaultUploadPath}s--Ai4Znfl3--/c_crop,h_20,w_10/v1234/image.jpg";
      //
      // var actual = cloudinarySignedUrl.image(options: ImageBuilder() ..version = '1234 ..Resize.crop(width(10); height(20)').generate(image.jpg);
    ...
    group('Scheme tests', () {
      test('Cloudinary url valid scheme success', () {
        var cloudinaryUrl = 'cloudinary://123456789012345:ALKJdjklLJAjhkKJ45hBK92baj3@test';
        CloudinaryConfig.fromUri(cloudinaryUrl);
      });

      test('Test cloudinary URL with invalid scheme throws', () {
        var cloudinaryUrl = 'https://123456789012345:ALKJdjklLJAjhkKJ45hBK92baj3@test';
        expect(() => CloudinaryConfig.fromUri(cloudinaryUrl),
            throwsArgumentError);
      });

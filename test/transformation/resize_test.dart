...
void main() {
  ...
  test('Test successful pad formatting', () {
    ...
    cldAssert('c_pad,h_100,w_100', Resize.pad(options: PadBuilder() ..width(100) ..height(100)));
    cldAssert('c_pad,h_100,w_100', Resize.pad(width: 100, height: 100));
    cldAssert('c_pad,h_1.1,w_0.5', Resize.pad(options: PadBuilder() ..width(0.5) ..height(1.1)));
    cldAssert('ar_1.5,c_pad,w_100', Resize.pad(options: PadBuilder() .. width(100) ..aspectRatio(1.5)));
    cldAssert('ar_1.5,c_pad,h_100', Resize.pad(options: PadBuilder() .. height(100) ..aspectRatio(1.5)));
    cldAssert('ar_1.5,c_pad,g_north,h_100', Resize.pad(options: PadBuilder() ..height(100) ..aspectRatio(1.5) ..gravity(Gravity.north())));
    cldAssert('ar_1.5,b_black,c_pad,h_100', Resize.pad(options: PadBuilder() ..height(100) ..aspectRatio(1.5) ..background(Background.color(Color.BLACK()))));
    cldAssert('ar_1.5,b_auto:predominant,c_pad,h_100', Resize.pad(options: PadBuilder() ..height(100) ..aspectRatio(1.5) ..background(Background.predominant())));
    cldAssert('ar_1.5,b_auto:border_contrast,c_pad,h_100', Resize.pad(options: PadBuilder() ..height(100) ..aspectRatio(1.5) ..background(Background.border(BorderBackgroundBuilder() ..contrast()))));
  });
}
